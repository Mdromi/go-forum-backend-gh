...
func TestCreatePost(t *testing.T) {
	...
	// Note: the value of the user password it was hashed is "password". so:
	password := "password"
	...
	// Nothe the author id is obtained from the token, so we dont pass it
	samples := []struct {
		inputJSON  string
		statusCode int
		title      string
...
func TestUpdatePost(t *testing.T) {
	...
	for _, v := range samples {
		...
		fmt.Println("responseInterface", rr.Body.String(), v.id)
